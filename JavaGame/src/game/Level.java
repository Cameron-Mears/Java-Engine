...
public class Level
{
    ...
    */
    ...
    public void renderMain(Graphics2D g, int xStart, int yStart, int xEnd, int yEnd, int xOffset, int yOffset)
    {
        for (int xIndex = xStart; xIndex < grid.length && xIndex < xEnd; xIndex++)
        {
            ...
            {
                if (grid[xIndex][yIndex] != null) grid[xIndex][yIndex].render(g, xOffset, yOffset);
                else {g.drawImage(background, xIndex * 32 + xOffset, yIndex * 32 + yOffset, null);}
            }
        }
    }
